...
import Testing
@testable import DekiValidator
import DekiParser

struct EmailValidationTests {

    let topLevelDomains: [String] = {
        if let domains = try? DekiParser.setupModel([String].self, fileName: "topLevelDomains", type: .yaml, bundle: .module) {
            return domains
        } else {
            return []
        }
    ...
    @Test func testEmailSimple() {
        ...
        for email in emails {
            #expect(DekiValidator.validateEmail(email))
        }
    }

    @Test func testEmailWithPlusSign() {
        let emails = [
        ...
        for email in emails {
            #expect(DekiValidator.validateEmail(email), "Email validation failed for \(email)")
        }
    }

    @Test func testEmailAlphaOnly() {
        let emails = [
            "a@b.cd",
            "a@b.cde",
            "user@example.com",
    ...
    @Test func testEmailAlphaNumericOnly() {
        let emails = [
            "a1@b.cd",
            "a2@b.cde",
            "us3er@example.com",
    ...
    @Test func testEmailWithAllowedSpecialCharacters() {
        let emails = [
            "!u!!s!er!@example.com",
            "#u##s#er#@example.com",
            "&u&&s&er&@example.com",
    ...
    @Test func testEmailCaseInsensitivity() {
        let validEmails = [
            "uSeR@example.com",
    ...
    @Test func testEmailWithSubDomain() {
        let emails = [
    ...
    @Test func testEmailWithDifferentTopLevelDomains() {
        let emails = [
            "user@example.co.uk",
            "contact@example.org",
    ...
    @Test func testEmailWithAllTopLevelDomains() {
        let validEmails = [
            "user@example.com.uk",
        ...
        for email in validEmails {
            for tld in self.topLevelDomains {
                ...
                #expect(DekiValidator.validateEmail(modifiedEmail), "Email validation failed for \(email) - \(modifiedEmail)")
            }
        }
    }
